# Smith's Lawn Care AI Assistant

AI-powered customer service assistant for Smith's Lawn Care businesses - handles chat, SMS, and appointment booking 24/7.
...
```bash
# Create database
sudo -u postgres psql -c "CREATE DATABASE smiths_lawn_care_crm;"
sudo -u postgres psql -c "CREATE USER hvacuser WITH ENCRYPTED PASSWORD 'your_password';"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE smiths_lawn_care_crm TO hvacuser;"

# Import schema
psql -U hvacuser -d smiths_lawn_care_crm -f smiths-lawn-care-crm-schema.sql
```
...
├── smiths-lawn-care-crm-schema.sql   # Database schema
...
See `smiths-lawn-care-crm-schema.sql` for the complete schema.
...
- GitHub Issues: https://github.com/dutchiono/smiths-lawn-care/issues
...
Built with ❤️ for Smith's Lawn Care businesses looking to automate customer service and grow their business.
